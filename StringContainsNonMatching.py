...
if __name__ == '__main__':
    ...
    file_name = "http_nonmatching_strings.input9"
...
# #     print("generation over")    
    ...
    for gen_str, str_len, match_pos_ratio in data:
        if str_len not in dict_str_len:
            dict_str_len[str_len] = 0
        dict_str_len[str_len] += 1    
        cmd = get_cmd(class_path, JAVA_CLASS_NAME, '_'.join(["nonmatching", str(str_len), str(dict_str_len[str_len])]), re.escape(SUBSTR_LITERAL), gen_str)
        cmds.append(cmd)

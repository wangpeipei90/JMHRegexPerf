...
def genStartsWith(start_str: str, size: int, len_lb=0, len_hb=1024, type="AlphaNumeric") -> List[str]:
    res = []
...
def genNotContains(contain_str: str, size: int, len_lb=0, len_hb=1024, type="AlphaNumeric"):
    ...
    while len(res) < size:
        ...
        res.extend(
            (gen_str, cur_len) for gen_str in generate_batch(unit, cur_len, type, exclude=lambda x: contain_str in x))
        cur_len += 1
    return res


def save_to_file(obj: List[str], filename: str) -> None:
    """for matched string with matching position"""
...
def save_to_file2(obj: List[str], filename: str) -> None:
    """for matched string with matching position"""
...
def asserted(obj: List[str], verify):
